...
void Connector::handleError()
{
	...
	if (state_ == kConnecting)
	{
		int sockfd = removeAndResetChannel();
		int err = sockets::getSocketError(sockfd);
		LOG_TRACE << "SO_ERROR = " << err << " " << strerrror_tl(err);
		retry(sockfd);
	}
}
...
void Connector::retry(int sockfd)
{
	...
	if (connect_)
	{
		LOG_INFO << "Connector::retry - Retry connecting to " << serverAddr_.toIpPort() << " in " << retryDelaysMs_ << " milliseconds. ";
