...
void muduo::net::defaultMessageCallback(const TcpConnectionPtr &conn, Buffer *buf, Timestamp)
{
	buf->retrieveAll();
}
...
void TcpConnection::sendInLoop(const void *data, size_t len)
{
	...
	if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0)
	{
		...
		if (!faultError && remaining > 0)
		{
			...
			{
				loop_->ququeInLoop(boost::bind(highWaterMarkCallback_, shared_from_this(), oldLen + remaining));
			}
...
void TcpConnection::connetionDestroyed()
{
	loop_->assertInLoopThread();
	if (state_ == kConnected)
	{
...
void TcpConnection::handleClose()
{
	...
	// we don't close fd, leave it to dtor, so we can find leaks easyily
	setState(kDisconnected);
	channel_->disableAll();
